...
#define Title_DefaultColor              [UIColor whiteColor]

//#define TITLE_LABEL_FONT                [UIFont fontWithName:@"Roboto-Regular" size:14]

#define TITLE_LABEL_FONT                [UIFont fontWithName:@"System-Regular" size:16]
...
- (void)addButtons {
    ...
    for (NSInteger number=0; number < numberOfItems; number++) {
        ...
        // Set font
        if (!self.slideFont) {
            self.slideFont = TITLE_LABEL_FONT;
        }
        
        [[(UIButton *)[_buttonsArray objectAtIndex:number] titleLabel] setFont:self.slideFont];
        self.previousButton = [_buttonsArray objectAtIndex:0];
    }
}
...
- (void)p_animateSliderWithTag:(NSInteger)tag {
    ...
    if(self.isFromBeatPlan){
        [_contentScrollView setContentOffset:CGPointMake(SCREEN_WIDTH * tag, 0) animated:NO];
    }else{
...
//    NSLog(@"比值 %f", offset.x/SCREEN_WIDTH);
    ...
    UIButton *currentButton = [_buttonsArray objectAtIndex:buttonTag];
    if (currentButton != self.previousButton && [self.delegate respondsToSelector:@selector(didSlideToButton:)]) {
        [self.delegate didSlideToButton:[_buttonsArray objectAtIndex:buttonTag]];
...
//    [button setTitleColor:[_slider backgroundColor] forState:UIControlStateNormal];
    ...
    if(self.isFromBeatPlan){
        [self.contentScrollView setContentOffset:CGPointMake(button.tag*SCREEN_WIDTH, 0) animated:NO];
    }else{
