...
//        NSString *str = [NSString stringWithFormat:@"%li-%li-%li",(long)components.day,(long)components.month, (long)components.year ];
        cell.cellDate = date;
        [cell setExclusionViewModel:_exclusionViewModel];
        [cell setArrayEvents:[dictEvents objectForKey:date]];
        [cell.labelDay setText:[NSString
                                stringWithFormat:@"%li", (long)[components day]]];
        
        if ([NSDate isTheSameDateTheCompA:components compB:[NSDate componentsOfCurrentDate]]) {
            [cell markAsCurrentDay];
        }
        if ([_exclusionViewModel.selectedDates containsObject:cell.cellDate]) {
            cell.tag = 1;
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    id obj = [[array objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    if (obj != [NSNull null]) {
...
//    FFMonthCell *cell = (FFMonthCell *)[collectionView cellForItemAtIndexPath:indexPath];
//    [popoverControllerAdd presentPopoverFromRect:cell.frame
//                                          inView: collectionView
//                        permittedArrowDirections:UIPopoverArrowDirectionAny
//                                        animated:YES];
//        //    [protocol showPopOverWithIndex:indexPath];
//        [popoverControllerAdd setPassthroughViews:nil];
//        dispatch_async(dispatch_get_main_queue(), ^{
//            [popoverControllerAdd presentPopoverFromRect:CGRectMake(20,20, 0, 0) inView:collectionView
...
-(void)showAllEventOfCell:(UICollectionViewCell *)cell atIndex:(NSInteger)intIndex{
    [protocol showAllEventOfCell:cell atIndex:intIndex];
}
/*
